...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header("Location: " . $_SERVER['HTTP_REFERER'] ."#".$tabela. "&status=error&msg=" . urlencode("Método de requisição inválido."));
    exit();
}
...
if (!$tabela || !preg_match('/^[a-zA-Z0-9_]+$/', $tabela)) {
    header("Location: " . $_SERVER['HTTP_REFERER'] ."#".$tabela. "&status=error&msg=" . urlencode("Tabela inválida ou não informada."));
    exit();
}
...
try {
    ...
    if ($action === 'create') {
        ...
        header("Location: " . $_SERVER['HTTP_REFERER'] . "#".$tabela."&status=success&msg=" . urlencode("Registro criado com sucesso."));
        exit();

    } elseif ($action === 'update') {
        $id = $_POST['id'] ?? null;
        if (!$id || !is_numeric($id)) {
            header("Location: " . $_SERVER['HTTP_REFERER'] ."#".$tabela. "&status=error&msg=" . urlencode("ID inválido."));
            exit();
        }
        ...
        header("Location: " . $_SERVER['HTTP_REFERER'] ."#".$tabela. "&status=success&msg=" . urlencode("Registro atualizado com sucesso."));
        exit();
        ...
        header("Location: " . $_SERVER['HTTP_REFERER'] ."#".$tabela. "&status=success&msg=" . urlencode("Registro excluído com sucesso."));
        exit();
    }

    header("Location: " . $_SERVER['HTTP_REFERER'] ."#".$tabela. "&status=error&msg=" . urlencode("Ação inválida."));
    exit();

} catch (Exception $e) {
    header("Location: " . $_SERVER['HTTP_REFERER'] ."#".$tabela. "&status=error&msg=" . urlencode($e->getMessage()));
    exit();
}
?>
